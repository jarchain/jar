...
pub struct Interpreter {
    ...
    /// Used for O(1) skip lookups instead of O(25) linear scans.
...
impl Interpreter {
    ...
    fn skip(&self, i: usize) -> usize {
        // skip(i) = min(24, dist from i+1 to next bitmask=1)
...
///
/// This replaces O(25) linear scans in skip_at/skip() with O(1) lookups
/// after a single O(n) backward pass.
    ...
    while pc < len {
        ...
        if pc < bitmask.len() && bitmask[pc] == 1 {
            if let Some(opcode) = Opcode::from_byte(code[pc]) {
                // O(1) skip lookup from pre-computed table (replaces O(25) linear scan)
