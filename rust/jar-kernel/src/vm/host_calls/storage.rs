...
//! `SnapshotStorage` cap held in the running VM's cap-table.
...
pub fn host_storage_read<H: Hardware>(
    ...
    // φ[7] = cap-table slot for Storage / SnapshotStorage cap
    ...
    match storage::storage_read(ctx.state, &cap, &key)? {
        Some(value) => {
...
pub fn host_storage_write<H: Hardware>(
    ...
    match storage::storage_write(ctx.state, &cap, &key, &val) {
...
pub fn host_storage_delete<H: Hardware>(
    ...
    match storage::storage_delete(ctx.state, &cap, &key) {
        Ok(()) => Ok(HostCallOutcome::Resume(RC_OK, 0)),
        Err(KernelError::ReadOnly(_)) => Ok(HostCallOutcome::Resume(RC_READONLY, 0)),
        Err(e) => Err(e),
    }
